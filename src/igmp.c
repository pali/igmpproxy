...
void acceptIgmp( int recvlen )
{
            ...
            for ( i = 0; i < MAX_UPS_VIFS; i++ ) {
                if ( -1 != upStreamIfIdx[i] ) {
                    ...
                    if ( checkVIF == 0 ) {
                        ...
                        my_log( LOG_NOTICE, 0, "Route activation request from %s for %s is "
                        ...
                        if ( downVIF && downVIF->state & IF_STATE_DOWNSTREAM ) {
                            my_log( LOG_NOTICE, 0, "The source address %s for group %s is "
                                                   "from downstream VIF[%d]. Ignoring.",
                                    inetFmt( src, s1 ), inetFmt( dst, s2 ), i );
                        }
                        else {
                            my_log( LOG_WARNING, 0, "The source address %s for group %s, is "
    ...
    if ( iphdrlen + ipdatalen != recvlen ) {
        my_log( LOG_WARNING, 0, "received packet from %s shorter (%u bytes) than "
                                "hdr+data length (%u+%u)",
