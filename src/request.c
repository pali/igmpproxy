...
void acceptGroupReport( uint32_t src, uint32_t group )
{
    ...
    if ( sourceVif->state == IF_STATE_DOWNSTREAM ) {
        ...
        my_log( LOG_DEBUG, 0, "Should insert group %s (from: %s) to route table. Vif Ix : %d", inetFmt( group, s1 ),
                inetFmt( src, s2 ), sourceVif->index );
        ...
        my_log( LOG_INFO, 0, "The group address %s may not be requested from this "
                             "interface. Ignoring.",
                inetFmt( group, s1 ) );
    }
    else {
...
void sendGroupSpecificMemberQuery( void *argument )
{
    ...
    /**
     * FIXME: This loops through all interfaces the group is active on an sends
     * queries.
     *        It might be better to send only a query on the interface the leave
     * was accepted on and remove only that
